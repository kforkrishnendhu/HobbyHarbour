...
namespace HobbyHarbour.Controllers;
...
public class HomeController : Controller
{
    ...
    public HomeController(ILogger<HomeController> logger, UserManager<ApplicationUser> userManager, SignInManager<ApplicationUser> signInManager, ApplicationDbContext db)
    {
        _logger = logger;
    ...
    {
        List<Product> objProductList = _db.Products.Include(p => p.Category).Include(p => p.Images).Where(p => !p.IsDeleted && p.Category.CategoryName == "Board Games").ToList();

        return View("Index", objProductList);
    }

    public IActionResult OutdoorToys()
    {
        List<Product> objProductList = _db.Products.Include(p => p.Category).Include(p => p.Images).Where(p => !p.IsDeleted && p.Category.CategoryName == "Outdoor Toys").ToList();

        return View("Index", objProductList);
    }

    public IActionResult RemoteToys()
    {
        List<Product> objProductList = _db.Products.Include(p => p.Category).Include(p => p.Images).Where(p => !p.IsDeleted && p.Category.CategoryName == "Remote Controls").ToList();

        return View("Index", objProductList);
    }

    public IActionResult SoftToys()
    {
        List<Product> objProductList = _db.Products.Include(p => p.Category).Include(p => p.Images).Where(p => !p.IsDeleted && p.Category.CategoryName == "Soft Toys").ToList();
